...
use App\Http\Requests\FrontendDashboard\MainSliderRequest;
...
class MainSliderController extends Controller
{
    ...
    {
        if (!auth()->user()->ability('frontend_dashboard', 'manage_main_sliders , show_main_sliders')) {
        ...
        return view('frontend_dashboard.main_sliders.index', compact('mainSliders'));
    }

    public function create()
    {
        if (!auth()->user()->ability('frontend_dashboard', 'create_main_sliders')) {
            return redirect('frontend_dashboard/index');
        }

        $tags = Tag::whereStatus(1)->get(['id', 'name']);

        return view('frontend_dashboard.main_sliders.create', compact('tags'));
    }
    ...
    public function store(MainSliderRequest $request)
    {
        ...
        if ($mainSlider) {
            return redirect()->route('frontend_dashboard.main_sliders.index')->with([
    ...
    public function show($id)
    {
        ...
        return view('frontend_dashboard.main_sliders.show');
    }


    public function edit($mainSlider)
    {
        if (!auth()->user()->ability('frontend_dashboard', 'update_main_sliders')) {
            return redirect('frontend_dashboard/index');
        }

        $mainSlider =  Slider::where('id', $mainSlider)->first();
        $tags = Tag::whereStatus(1)->get(['id', 'name']);
        return view('frontend_dashboard.main_sliders.edit', compact('tags', 'mainSlider'));
    }
    ...
    {
        if (!auth()->user()->ability('frontend_dashboard', 'delete_main_sliders')) {
            return redirect('frontend_dashboard/index');
        }
