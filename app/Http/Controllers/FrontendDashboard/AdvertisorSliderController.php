...
use App\Http\Requests\FrontendDashboard\AdvertisorSliderRequest;
...
class AdvertisorSliderController extends Controller
{
    ...
    {
        if (!auth()->user()->ability('frontend_dashboard', 'manage_advertisor_sliders , show_advertisor_sliders')) {
        ...
        return view('frontend_dashboard.advertisor_sliders.index', compact('advertisorSliders'));
    }

    public function create()
    {
        if (!auth()->user()->ability('frontend_dashboard', 'create_advertisor_sliders')) {
            return redirect('frontend_dashboard/index');
        }

        $tags = Tag::whereStatus(1)->get(['id', 'name']);

        return view('frontend_dashboard.advertisor_sliders.create', compact('tags'));
    }
    ...
    public function store(AdvertisorSliderRequest $request)
    {
        ...
        if ($advertisorSlider) {
            return redirect()->route('frontend_dashboard.advertisor_sliders.index')->with([
    ...
    {
        if (!auth()->user()->ability('frontend_dashboard', 'display_sliders')) {
            return redirect('frontend_dashboard/index');
        }

        return view('frontend_dashboard.advertisor_sliders.show');
    }

    public function edit($advertisorSlider)
    {
        if (!auth()->user()->ability('frontend_dashboard', 'update_advertisor_sliders')) {
            return redirect('frontend_dashboard/index');
        }

        $advertisorSlider =  Slider::where('id', $advertisorSlider)->first();
        $tags = Tag::whereStatus(1)->get(['id', 'name']);
        return view('frontend_dashboard.advertisor_sliders.edit', compact('tags', 'advertisorSlider'));
    }
    ...
    {
        if (!auth()->user()->ability('frontend_dashboard', 'delete_advertisor_sliders')) {
            return redirect('frontend_dashboard/index');
        }
