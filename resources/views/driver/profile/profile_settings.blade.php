...
<form action="{{ route('driver.profile.update', auth()->user()->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="file" name="user_image" id="driver_profile_image" class="file-input-overview ">
                            @error('user_image')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    @ability('driver', 'update_account_settings')
                        <div class="text-end">
...
@section('script')
    ...
    <script>
        $(function() {
            $("#driver_profile_image").fileinput({
                theme: "fa5",
                maxFileCount: 1,
                    ...
                    @if (auth()->user()->user_image != '')
                        {
                            ...
                            size: '1111',
                            width: "120px",
                            url: "{{ route('driver.profile.remove_image', ['driver_id' => auth()->user()->id, '_token' => csrf_token()]) }}",
                            key: {{ auth()->user()->id }}
                        }
                    @endif
                ]
