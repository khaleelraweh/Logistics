{{-- resources/views/driver/return_requests/partials/sender-info.blade.php --}}
<div class="row">
    <div class="col-md-6 mb-3">
        <div class="info-label">{{ __('general.full_name') }}</div>
        <div class="info-value fw-semibold">{{ $package->sender_full_name }}</div>
    </div>
    <div class="col-md-6 mb-3">
        <div class="info-label">{{ __('general.phone') }}</div>
        <div class="info-value">
            <i class="mdi mdi-phone-outline me-2"></i>
            +{{ $package->sender_phone }}
        </div>
    </div>
    <div class="col-12 mb-3">
        <div class="info-label">{{ __('general.email') }}</div>
        <div class="info-value">
            <i class="mdi mdi-email-outline me-2"></i>
            {{ $package->sender_email ?? '-' }}
        </div>
    </div>
    <div class="col-12 mb-3">
        <div class="info-label">{{ __('general.address') }}</div>
        @php
            $senderAddressParts = array_filter([
                $package->sender_district,
                $package->sender_city,
                $package->sender_region,
                $package->sender_country,
                $package->sender_postal_code,
            ]);
            $fullSenderAddress = implode(' ، ', $senderAddressParts);
        @endphp
        <div class="info-value">
            <i class="mdi mdi-map-marker-outline me-2"></i>
            {{ $fullSenderAddress }}
        </div>
    </div>
    <div class="col-md-4 mb-3">
        <div class="info-label">{{ __('general.city') }}</div>
        <div class="info-value">{{ $package->sender_city }}</div>
    </div>
    <div class="col-md-4 mb-3">
        <div class="info-label">{{ __('general.region') }}</div>
        <div class="info-value">{{ $package->sender_region }}</div>
    </div>
    <div class="col-md-4 mb-3">
        <div class="info-label">{{ __('general.country') }}</div>
        <div class="info-value">{{ $package->sender_country }}</div>
    </div>
    <div class="col-md-6 mb-3">
        <div class="info-label">{{ __('general.latitude') }}</div>
        <div class="info-value coordinates">{{ $package->sender_latitude }}</div>
    </div>
    <div class="col-md-6 mb-3">
        <div class="info-label">{{ __('general.longitude') }}</div>
        <div class="info-value coordinates">{{ $package->sender_longitude }}</div>
    </div>
    @if($package->sender_notes)
    <div class="col-12 mt-3">
        <div class="alert alert-light border">
            <div class="info-label">{{ __('general.notes') }}</div>
            <div class="info-value">{{ $package->sender_notes }}</div>
        </div>
    </div>
    @endif
</div>
