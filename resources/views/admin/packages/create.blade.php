    ...
    <div class="row ">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between">
                <h4 class="mb-0 font-size-18">{{ __('package.add_new_package') }}</h4>
...
{{-- @section('script')
    ...
    <!-- حل مشكلة الخريطة في التبويبات -->
    <script>
        $(document).ready(function() {
            // إعادة رسم الخرائط عند التبديل بين التبويبات
            $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
                setTimeout(() => {
                    // إرسال حدث لإعادة رسم جميع الخرائط
                    window.dispatchEvent(new Event('resize'));

                    // إعادة رسم خرائط Livewire
                    if (typeof Livewire !== 'undefined') {
                        Livewire.components.components().forEach(component => {
                            if (component.name.includes('location-component')) {
                                Livewire.emitTo(component.name, 'refreshMap');
                            }
                        });
                    }
                }, 400);
            ...
            // بدء مراقبة عنصر الخريطة
            const mapElements = document.querySelectorAll('#sender-map, #receiver-map');
        ...
        // دالة عالمية لإعادة رسم الخرائط
    ...
    $(document).ready(function() {
        ...
        $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
            setTimeout(() => {
                ...
                // إعادة رسم خرائط Livewire المحددة
                if (typeof Livewire !== 'undefined') {
                    Livewire.emit('refreshMap'); // للمرسل
                    Livewire.emit('refreshReceiverMap'); // للمستلم
                }
            }, 400);
        });
