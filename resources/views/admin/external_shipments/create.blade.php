...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0">{{ __('external_shipment.add_external_shipment') }}</h4>
            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item">
                        <a href="{{ route('admin.external_shipments.index') }}">{{ __('external_shipment.manage_external_shipments') }}</a>
                    </li>
                    <li class="breadcrumb-item active">{{ __('external_shipment.add_external_shipment') }}</li>
                </ol>
            </div>
        </div>
    </div>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <form action="{{ route('admin.external_shipments.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="col-sm-10">
                            <select name="shipping_partner_id" class="form-control select2">
                                ...
                                @foreach($partners as $partner)
                                    <option value="{{ $partner->id }}" {{ old('shipping_partner_id') == $partner->id ? 'selected' : '' }}>
                                        {{ $partner->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="external_tracking_number" class="form-control" id="external_tracking_number" value="{{ old('external_tracking_number') }}">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="date" name="delivery_date" class="form-control" id="delivery_date" value="{{ old('delivery_date') }}">
                            @error('delivery_date')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <select name="status" id="status1" class="form-control select2">
                                <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>{{ __('external_shipment.status_pending') }}</option>
                                <option value="cancelled" {{ old('status') == 'cancelled' ? 'selected' : '' }}>{{ __('external_shipment.status_cancelled') }}</option>
                                <option value="in_transit" {{ old('status') == 'in_transit' ? 'selected' : '' }}>{{ __('external_shipment.status_in_transit') }}</option>
                                <option value="delivered" {{ old('status') == 'delivered' ? 'selected' : '' }}>{{ __('external_shipment.status_delivered') }}</option>
                            </select>
                            @error('status')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                    ...
                    {{-- Submit --}}
                    @ability('admin', 'create_external_shipments')
                        <div class="text-end">
                            <button type="submit" class="btn btn-primary">{{ __('external_shipment.save_external_shipment') }}</button>
                        </div>
                    @endability
